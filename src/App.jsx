
// Importamos Componentes
import './App.css'
import MeuNome from './components/MeuNome';   // componente com props
import OlaMundo from './components/OlaMundo'; // componente sem props
import Pessoa from './components/Pessoa';

function App() {
  // Logica 
  // logica basica 
  let nome = "Leximibel Langa" // Ex: uma variavel nome que vai pegar o nome do usuario directo do navegador. 

  // logica com funcoes
  function anoActual() {
    return new Date().getFullYear(); 
  }
  function calcularIdade(anoNasc) {
    return anoActual() - anoNasc;
  }

  // Imagem vindo de outro site
  const urlImagem = "data:image/jpeg;base64,";


  // Resultado 
  return (
    <div className='App'>
      {/* Primeiro teste */}
      <h1>Ola, React</h1>
      <p>Meu primeiro App</p>

      {/* Exibir o nome que vem do servidor */}
      <p>O nome que veio do servidor eh: <strong>{nome}</strong></p>


      <hr />
      <br /><br />
      <div className="App-formatacao">
        <h2>Formatando o nome</h2>
        {/* Formatacoes do nome */}
        <p>Maiusculo:      <strong>{nome.toUpperCase()}</strong></p>
        <p>Minusculo:      <strong>{nome.toLowerCase()}</strong></p>
        <p>Primeira letra: <strong>{nome.charAt(0)}</strong></p>
        <p>Ultima letra:   <strong>{nome.charAt(nome.length - 1)}</strong></p>
        <p>Tamanho:        <strong>{nome.length}</strong></p>

        <br />
        {/* Operacoes do JS */}
        <h2>Opreracoes do JS</h2>
        <p>Calculando a minha idade: <strong>{calcularIdade(2005)}</strong> <br /> <small>Para efectuar esse calculo usei o ano actual({anoActual()}) e o ano do meu nascimento(2005).</small></p>

        <br />
        {/* Imagem dinamica */}
        <div className="App-img">
          <h2>Imagem dinamica</h2>
          <img className='appImg' src={urlImagem} alt='Minha imagem dinamica' />
        </div>


        <br /><br />
        <hr />
        <h4>Componente sem props *</h4>
        {/* Componentes */}
        <OlaMundo />


        <br /><br />
        <hr />
        <h4>Componente com props *</h4>
        {/* Componente com Props */}
        <MeuNome nome="Leximibel" />

        <br /><br />

        {/* Mais de uma prop */}
        <Pessoa nome="Chelddy" idade="15" foto="https://via.placeholder.com/150" profissao="Programador" />

         

      </div>
    </div>
  )
}


// Enviar o Resultado
export default App
